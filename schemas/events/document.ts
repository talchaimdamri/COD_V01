...
export class DocumentEventFactory {
   ...
   */
  static createContentChangeEvent(
    documentId: string,
  ...
  ): z.infer<typeof DocumentContentChangeEventSchema> {
   ...
   */
  static createTitleChangeEvent(
    documentId: string,
    previousTitle: string,
    newTitle: string,
    options?: { userId?: string }
  ): z.infer<typeof DocumentTitleChangeEventSchema> {
    return {
  ...
  ): z.infer<typeof DocumentVersionSaveEventSchema> {
   ...
   */
  static createConnectionChangeEvent(
    documentId: string,
    connectionId: string,
  ...
  ): z.infer<typeof DocumentConnectionChangeEventSchema> {
    return {
   ...
   */
  static createFormattingChangeEvent(
    documentId: string,
    position: { from: number; to: number },
    formatType: 'bold' | 'italic' | 'underline' | 'strike' | 'code' | 'heading' | 'paragraph' | 'list' | 'blockquote' | 'codeblock',
    formatAction: 'apply' | 'remove' | 'toggle',
    options?: {
      formatData?: Record<string, any>
      userId?: string
    }
  ): z.infer<typeof DocumentFormattingChangeEventSchema> {
    return {
  ...
  ): z.infer<typeof DocumentUndoRedoEventSchema> {
    return {
  ...
  ): z.infer<typeof DocumentSessionEventSchema> {
    return {
...
export class DocumentEventUtils {
   ...
   */
  static isContentChangeEvent(event: any): event is z.infer<typeof DocumentContentChangeEventSchema> {
   ...
   */
  static isStructuralChangeEvent(event: any): boolean {
    return [
   ...
   */
  static isVersionEvent(event: any): event is z.infer<typeof DocumentVersionSaveEventSchema> {
   ...
   */
  static calculateContentStats(events: any[]): {
    totalChanges: number
    insertions: number
    deletions: number
    wordCount: number
    ...
    for (const event of events) {
      if (this.isContentChangeEvent(event)) {
        ...
        if (changeType === 'insert') insertions++
        else if (changeType === 'delete') deletions++
   ...
   */
  static getVersionHistory(events: any[]): Array<{
    version: number
    timestamp: Date
    description?: string
    wordCount: number
...
/**
 * TypeScript types for document events
 */
...
export type DocumentUndoRedoEvent = z.infer<typeof DocumentUndoRedoEventSchema>
export type DocumentSessionEvent = z.infer<typeof DocumentSessionEventSchema>
